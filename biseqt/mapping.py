...
class Read(object):
    def __init__(self, db, record, rc_record):
...
class ReadMapper(object):
    ...
    def load_reads(self):
        ...
        for record in recs_by_content_id.values():
            if 'rc_of' in record.attrs:
                ...
                reads.append(Read(self.db, *pair))
    ...
    def map_read(self, read, targets, min_band_score=20, **aligner_kw):
        ...
        with Aligner(seq, target_seq, **aligner_kw) as aligner:
            ...
            if alignment is not None:
                ours = read.record if not rc else read.rc_record
    ...
    def map_all_to_all(self, min_band_score, **aligner_kw):
        ...
        for read in reads:
            ...
            for other in others:
                rec, target_rec, aln = self.map_read(other,
                    min_band_score=min_band_score, **aligner_kw)
                if rec is None:
                    continue
    ...
    def map_all_to_refs(self, min_band_score, **aligner_kw):
        ...
        for read in reads:
            indic.progress()
            rec, target_rec, aln = self.map_read(refs,
                min_band_score=min_band_score, **aligner_kw)
            if rec is not None:
                yield rec, target_rec, aln
        indic.finish()
