...
int dptable_init(dptable* T) {
  ...
  switch (T->mode) {
    case STD_MODE:
      framedims = _framedims(T->std_prob->frame);
      T->num_rows = framedims.i + 1;
      T->num_cols = framedims.j + 1;
      break;
    case BANDED_MODE:
      framedims = _framedims(T->std_prob->frame);
      T->num_rows = 1 + 2 * T->banded_prob->bradius;
      T->num_cols = 1 + (framedims.i < framedims.j ? framedims.i : framedims.j);
      T->row_lens = malloc(T->num_rows * sizeof(int));
      for (i = 0; i < T->num_rows; i++) {
        T->row_lens[i] = _da_row_len(framedims, i + T->banded_prob->bradius);
      }
      break;
  }
...
intpair dptable_solve(dptable* T) {
  ...
  intpair cellpos, framedims; // dpcell position in the table, could be either xy/da
  int i,j,k;
  int *max_score_choices = NULL;
  double max_score;
  alnchoice *choices = NULL;

  framedims = _framedims(T->mode == STD_MODE ? T->std_prob->frame : T->banded_prob->frame);
  ...
  // coordinate system).
  for (i = 0; i < framedims.i + 1; i++) {
    for (j = 0; j < framedims.j + 1; j++) {
      // the coordinates of our cell in the coordinate system in use: xy or da.
      cellpos = _cellpos_from_xy(i, j, T->mode, framedims.j);
      if (choices != NULL) {
        free(choices);
      }
  ...
  free(choices);
  free(max_score_choices);
  return (T->mode == STD_MODE ? _std_find_optimal(T) : _banded_find_optimal(T));
}
...
transcript* dptable_traceback(dptable* T, intpair end) {
  ...
  if (T->mode == STD_MODE) {
    ...
    starts = _xy_from_da(end.i - T->banded_prob->bradius, end.j);
  }
  ...
  tx->S_idx = starts.i + frame->S_min_idx;
  tx->T_idx=starts.j + frame->T_min_idx;
