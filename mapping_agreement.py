...
bwa = load_mappings('leishmania/ours.mappings.txt')
...
for i,j in combinations(mapped, 2):
    bwa_overlap = bwa[i].rc == bwa[j].rc and not (bwa[i].ref_from > bwa[j].ref_to or bwa[i].ref_to > bwa[j].ref_from)
    blasr_overlap = blasr[i].rc == blasr[j].rc and not (blasr[i].ref_from > blasr[j].ref_to or blasr[i].ref_to > blasr[j].ref_from)
