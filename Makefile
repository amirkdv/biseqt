...
test.fa:
	python -c "from align import randseq as R; x = R.gen(60); print '> orig\n' + x; print '\n'.join(['> seq_%d\n' % i + R.mutate(x, gap_open=0.1, rates={i:{k:0.94 if k==i else 0.02 for k in 'ACGT'} for i in 'ACGTG'})[0] for i in range(10)]);" > /tmp/$@
