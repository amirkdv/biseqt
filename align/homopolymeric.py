...
class HpCondenser(object):
    ...
    def condense_seed(self, S, T, seed):
        ...
        Note:
            If the provided seed is not found by an :class:`HpCondensedIndex`
            (e.g an ordinary :class:`align.tuples.Index`) then the seed
            boundaries may not coincide with homopolymeric stretch boundaries.
            If so, an ``AssertionError`` is raised since the seed is not
            translatable as-is to the condensed alphabet.
        """
